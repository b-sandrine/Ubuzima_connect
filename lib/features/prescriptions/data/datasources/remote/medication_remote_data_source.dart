...
///
/// [patientId] is the signed-in patient's own uid, so each real patient gets
/// their own document rather than sharing one demo record. On first read a
/// new patient's record is seeded from [MedicationLocalDataSource] — using
/// their real display name — so the screen always has live data to work
/// against even before a real prescription-issuing flow exists; "Take" and
/// "Request refill" then write straight to Firestore.
...
class MedicationRemoteDataSourceImpl implements MedicationRemoteDataSource {
  ...
  String get _patientId => _auth.currentUser?.uid ?? AppConstants.demoPatientId;

  DocumentReference<Map<String, dynamic>> get _doc =>
      _firestore.collection(FirestorePaths.prescriptions).doc(_patientId);
  ...
  /// Writes the canonical demo schedule into Firestore once, so a patient
  /// who has no real prescriptions yet still sees a live schedule — but
  /// under their own real name, not the seed template's.
  Future<void> _seedFirestore() async {
    final schedule = _seed.readTodaySchedule();
    final batch = _firestore.batch();
    final realName = _auth.currentUser?.displayName?.trim();
    final patientName = (realName == null || realName.isEmpty)
        ? schedule.summary.patientName
        : realName;
